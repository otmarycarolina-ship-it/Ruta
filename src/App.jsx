...
const App = () => {
  ...
  const temas = {
    gradienteEstatico: {
      ...
      primaryLight: "bg-slate-100/80 dark:bg-zinc-800/60",
      primaryBorder: "border-indigo-100 dark:border-zinc-800/80",
      accent: "text-[#e44d9b] dark:text-pink-400",
      accentBg: "bg-gradient-to-r from-purple-50 via-pink-50 to-rose-50 dark:from-purple-950/30 dark:via-pink-950/30 dark:to-rose-950/30",
      buttonHover: "hover:brightness-105",
      gradient: "from-[#7a57d1] to-[#e44d9b]",
      bgOverlay: "bg-slate-50 dark:bg-[#0b141a]",
    ...
    sakura: {
      ...
      bgOverlay: "bg-pink-50 dark:bg-[#0b141a]",
    ...
    morado: {
      ...
      bgOverlay: "bg-purple-50 dark:bg-[#0b141a]",
    ...
    verde: {
      ...
      bgOverlay: "bg-emerald-50 dark:bg-[#0b141a]",
    ...
    naranja: {
      ...
      bgOverlay: "bg-orange-50 dark:bg-[#0b141a]",
    ...
    amarillo: {
      ...
      bgOverlay: "bg-amber-50/40 dark:bg-[#0b141a]",
    ...
    rojo: {
      ...
      bgOverlay: "bg-red-50/50 dark:bg-[#0b141a]",
    ...
    <div className={`min-h-screen ${t.bgOverlay} p-4 md:p-10 font-sans text-slate-700 dark:text-zinc-300 relative overflow-x-hidden transition-all duration-700`}>
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2 bg-white/90 dark:bg-[#1f2c34] backdrop-blur-sm p-6 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-zinc-800/50 flex items-center justify-between">
            <button onClick={() => cambiarMes(-1)} className={`p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800/40 transition-colors ${temaActual === 'gradienteEstatico' ? 'text-[#7a57d1] dark:text-[#a892e8]' : t.primary}`}><ChevronLeft size={28} /></button>
            <div className="text-center">
              <h2 className="text-2xl font-bold text-slate-800 dark:text-white">{mesActualKey}</h2>
              <p className={`text-[10px] font-bold ${t.accent} tracking-[0.2em] uppercase transition-colors`}>{anioActual}</p>
            </div>
            <button onClick={() => cambiarMes(1)} className={`p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800/40 transition-colors ${temaActual === 'gradienteEstatico' ? 'text-[#e44d9b] dark:text-pink-400' : t.primary}`}><ChevronRight size={28} /></button>
          </div>
          ...
          <div className={`p-6 rounded-[2.5rem] shadow-lg text-white flex flex-col justify-center relative overflow-hidden transition-all duration-700 ${t.primaryBg}`}>
            ...
            <div className="flex items-center gap-3 z-10">
              <input type="number" value={currentData.meta} onChange={(e) => updateCurrentMonth({ meta: Number(e.target.value) })} className="bg-white/20 w-20 text-3xl font-black rounded-xl text-center focus:outline-none placeholder-white/50 transition-all focus:bg-white/30 text-white"/>
              <span className="text-lg font-bold">horas</span>
            </div>
          </div>
        </div>

        <section className={`mb-8 bg-white/80 dark:bg-[#1f2c34] backdrop-blur-md border ${t.primaryBorder} p-6 rounded-[2.5rem] flex flex-wrap items-center justify-around gap-4 shadow-sm transition-colors`}>
          <div className="flex items-center gap-4">
            <div className={`p-4 rounded-2xl ${isTimerRunning ? `${t.primaryBg} animate-pulse shadow-lg shadow-current/20 text-white` : 'bg-slate-100 dark:bg-zinc-800 text-slate-400 dark:text-zinc-400'} transition-all`}>
              <Clock size={24} />
            </div>
            <p className={`text-4xl font-black font-mono tabular-nums transition-colors ${isTimerRunning && temaActual === 'gradienteEstatico' ? 'bg-gradient-to-r from-[#7a57d1] to-[#e44d9b] bg-clip-text text-transparent' : isTimerRunning ? t.primary : 'text-slate-700 dark:text-white'}`}>{formatTimer(secondsElapsed)}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 space-y-8">
            <section className="bg-white dark:bg-[#1f2c34] border border-transparent dark:border-zinc-800/50 p-8 rounded-[2.5rem] shadow-sm">
              <h3 className="text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest mb-6 flex items-center gap-2">Registro de hoy</h3>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-zinc-400 ml-2">HORAS</label>
                    <input type="number" placeholder="0" className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 text-2xl font-black text-slate-700 dark:text-white focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/50 outline-none transition-all placeholder-slate-300 dark:placeholder-zinc-600" value={nuevaHora} onChange={e => setNuevaHora(e.target.value)}/>
                </div>
                <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 dark:text-zinc-400 ml-2">MINS</label>
                    <input type="number" placeholder="0" className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 text-2xl font-black text-slate-700 dark:text-white focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/50 outline-none transition-all placeholder-slate-300 dark:placeholder-zinc-600" value={nuevoMinuto} onChange={e => setNuevoMinuto(e.target.value)}/>
                </div>
              </div>
            ...
            <section className="bg-white dark:bg-[#1f2c34] border border-transparent dark:border-zinc-800/50 p-8 rounded-[2.5rem] shadow-sm">
              <h3 className="text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest mb-6 flex items-center gap-2">Actividad Diaria</h3>
                ...
                {[...Array(totalDiasMes)].map((_, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`aspect-square rounded-xl text-[10px] font-bold transition-all ${tieneActividad ? `text-white shadow-md ${t.primaryBg}` : `bg-slate-50 dark:bg-[#121b22] text-slate-400 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800/40`}`}
                    >
                      {dia}
                    </button>
                  );
          ...
          <div className="lg:col-span-8 space-y-8">
            <section className="bg-white dark:bg-[#1f2c34] p-8 rounded-[3rem] shadow-xl border border-slate-100 dark:border-zinc-800/50 text-center">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="p-4 bg-slate-50 dark:bg-[#121b22] rounded-2xl transition-all hover:shadow-inner"><p className="text-[10px] font-bold text-slate-400 dark:text-zinc-400 uppercase mb-1">Total Horas</p><p className={temaActual === 'gradienteEstatico' ? "text-2xl font-black bg-gradient-to-r from-[#7a57d1] to-[#e44d9b] bg-clip-text text-transparent" : `text-2xl font-black ${t.primary} transition-colors`}>{horas}h {minutos}m</p></div>
                <div className="p-4 bg-slate-50 dark:bg-[#121b22] rounded-2xl transition-all hover:shadow-inner"><p className="text-[10px] font-bold text-slate-400 dark:text-zinc-400 uppercase mb-1">Cursos</p><p className={temaActual === 'gradienteEstatico' ? "text-2xl font-black text-[#e44d9b] dark:text-pink-400" : `text-2xl font-black ${t.primary} transition-colors`}>{currentData.estudiantes.length}</p></div>
            ...
            <section className="bg-white dark:bg-[#1f2c34] border border-transparent dark:border-zinc-800/50 p-8 rounded-[3rem] shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest flex items-center gap-2">Cursos Bíblicos</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {currentData.estudiantes.length > 0 ? (
                  currentData.estudiantes.map(est => (
                    <div key={est.id} onClick={() => {setFormEstudiante(est); setShowEditModal(est.id)}} className="p-5 bg-slate-50 dark:bg-[#121b22] rounded-[1.5rem] border border-transparent dark:border-zinc-800/40 flex justify-between items-center cursor-pointer hover:border-slate-200 dark:hover:border-zinc-700 hover:bg-white dark:hover:bg-[#1f2c34] hover:shadow-sm transition-all group">
                      <div>
                        <p className="font-bold text-slate-800 dark:text-white text-sm">{est.nombre}</p>
                        <p className="text-[10px] font-bold text-slate-400 dark:text-zinc-400 uppercase tracking-wider mt-1">
                  ...
                  <p className="col-span-full text-center py-8 text-slate-300 dark:text-zinc-500 text-sm italic">No hay cursos registrados</p>
                )}
              </div>
            </section>
          </div>
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-3">
        {showThemeSelector && (
            <div className="bg-white/95 dark:bg-[#1f2c34]/95 backdrop-blur-md p-3 rounded-3xl shadow-2xl border border-slate-100 dark:border-zinc-800 flex flex-col gap-3 max-h-[50vh] overflow-y-auto animate-in fade-in slide-in-from-bottom-4 zoom-in-95 scrollbar-none">
                {['sakura', 'gradienteEstatico', 'morado', 'azul', 'verde', 'naranja', 'amarillo', 'rojo'].map(key => (
                    <button 
                        ...
                        className={`w-10 h-10 rounded-2xl border-2 transition-all hover:scale-110 active:scale-90 flex-shrink-0 ${temaActual === key ? 'border-slate-800 dark:border-white' : 'border-transparent'}`}
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-[110] bg-slate-900/40 dark:bg-black/60 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-white dark:bg-[#1f2c34] w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 dark:border-zinc-800/50 animate-in slide-in-from-bottom-8 zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h4 className="text-xl font-bold text-slate-800 dark:text-white">{showEditModal === 'nuevo' ? 'Nuevo Curso' : 'Editar Curso'}</h4>
            ...
            <div className="space-y-4">
              <input type="text" placeholder="Nombre completo" className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 text-sm focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/40 outline-none transition-all text-slate-700 dark:text-white" value={formEstudiante.nombre} onChange={e => setFormEstudiante({...formEstudiante, nombre: e.target.value})}/>
              
              <div className="flex gap-2 w-full">
                <div className="w-[45%]">
                  <input type="text" placeholder="Día" className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 text-sm focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/40 outline-none transition-all text-slate-700 dark:text-white" value={formEstudiante.fecha} onChange={e => setFormEstudiante({...formEstudiante, fecha: e.target.value})}/>
                </div>
                ...
                <div className="relative flex items-center group/time w-[55%]">
                  ...
                  <input 
                    type="time" 
                    className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 pl-9 pr-3 text-sm focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/40 outline-none transition-all cursor-pointer text-slate-700 dark:text-white min-w-0" 
              ...
              <input type="text" placeholder="Capítulo / Lección" className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 text-sm focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/40 outline-none transition-all text-slate-700 dark:text-white" value={formEstudiante.leccion} onChange={e => setFormEstudiante({...formEstudiante, leccion: e.target.value})}/>
              <textarea placeholder="Observaciones..." rows="2" className="w-full bg-slate-50 dark:bg-[#121b22] rounded-2xl p-4 text-sm focus:ring-4 focus:ring-slate-100 dark:focus:ring-zinc-800/40 outline-none resize-none transition-all text-slate-700 dark:text-white" value={formEstudiante.notas} onChange={e => setFormEstudiante({...formEstudiante, notas: e.target.value})}/>
              <button onClick={() => {
