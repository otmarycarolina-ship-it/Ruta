                ...
                {currentData.estudiantes.length > 0 ? (
                  currentData.estudiantes.map(est => (
                    <div key={est.id} onClick={() => {setFormEstudiante(est); setShowEditModal(est.id)}} className="p-5 bg-slate-50 rounded-[1.5rem] border border-transparent flex justify-between items-center cursor-pointer hover:border-slate-200 hover:bg-white hover:shadow-sm transition-all group">
                      ...
                      <button onClick={(e) => {e.stopPropagation(); if(window.confirm(`¿Estás seguro que deseas borrar el curso de ${est.nombre}?`)) updateCurrentMonth({ estudiantes: currentData.estudiantes.filter(i => i.id !== est.id) })}} className="text-slate-400 hover:text-red-600 opacity-100 md:opacity-0 group-hover:opacity-100 transition-all p-1"><Trash2 size={16} /></button>
                    </div>
                  ))
                ) : (
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border border-slate-100 animate-in slide-in-from-bottom-8 zoom-in-95">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 w-full">
                <div className="w-[45%]">
                  <input type="text" placeholder="Día" className="w-full bg-slate-50 rounded-2xl p-4 text-sm focus:ring-4 focus:ring-slate-100 outline-none transition-all" value={formEstudiante.fecha} onChange={e => setFormEstudiante({...formEstudiante, fecha: e.target.value})}/>
                </div>
